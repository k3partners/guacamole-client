...
angular.module("index").controller("indexController", [
  ...
  function indexController($scope, $injector) {
    ...
    keyboard.onkeydown = function onkeydown(keysym) {
      ...
      if ($scope.applicationState !== ApplicationState.READY) return true;
    ...
    keyboard.onkeyup = function onkeyup(keysym) {
      ...
      // shown
      if ($scope.applicationState !== ApplicationState.READY) return;
    ...
    $window.addEventListener(
      ...
      function focusGained(e) {
        ...
        if (e.target === $window) clipboardService.resyncClipboard();
      },
      true
    );
    ...
    $scope.backToUserManagement = function backToUserManagement() {
      $translate("APP.USER_MANAGEMENT_URL").then((url) => {
